...
void fn_bs_destroy_Stan_model(Stan_model_struct &Stan_model_as_cpp_struct) {
  ...
  struct ParamConstrainWorker : public RcppParallel::Worker {
    ...
    // Constructor
    ParamConstrainWorker( const int n_threads_R,
                          const int n_iter_R,
                          const std::vector<Eigen::Matrix<double, -1, -1>> &unc_params_trace_input_main,
                          const std::vector<Eigen::Matrix<double, -1, -1>> &unc_params_trace_input_nuisance,
                          const Eigen::VectorXi  &pars_indicies_to_track,
      ...
      all_param_outs_trace(all_param_outs_trace)
    {
      ...
      // all_param_outs_trace_concurrent.reserve(n_threads_R);
      // for (int i = 0; i < n_threads_R; ++i) {
      //   all_param_outs_trace_concurrent[i](all_param_outs_trace[i]);
      //   // all_param_outs_trace_concurrent.emplace_back(all_param_outs_trace[i]);
      // }
    ...
    void operator()(std::size_t begin, std::size_t end) {
      ...
      std::size_t kk = begin;
      {
              ...
              for (int ii = 0; ii < n_iter; ii++) {
                    ...
                    if (result != 0) {
                      throw std::runtime_error("computation failed: " +  std::string(error_msg ? error_msg : "Unknown error"));
                    }
             ...
             //  all_param_outs_trace_concurrent[kk] =  fn_convert_EigenMat_to_RMatrix(chain_output,  all_param_outs_trace_concurrent[kk]);
              all_param_outs_trace[kk] = chain_output;
