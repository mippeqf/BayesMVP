...
# and probably not worth the extra overhead (even on a 96-core AMD EPYC Genoa CPU)
  ...
  if (Model_type == "LC_MVP") {
      ...
      model_args_list  <- list(       lkj_cholesky_eta = c(12, 3), 
                                      ...
                                      prior_coeffs_mean_vec = prior_a_mean,
                                      prior_coeffs_sd_vec =    prior_a_sd, 
  ...
  model_samples <-            ( model_obj$sample(    seed = 2,
                                                      n_iter = n_iter,
                                                      # vect_type = "Stan",
                                                      vect_type = "AVX512",
  ...
  #### trace <-  model_samples$result$result[[1]]
  ...
  # can now extract things from "model_summary" object, for example:
  ...
  time_total_MCMC <- model_efficiency_metrics$time_total_wo_summaries  ; time_total_MCMC
  time_total_inc_summaries <- model_efficiency_metrics$time_total ; time_total_inc_summaries # note this includes time to compute R-hat, etc 
  ...
  Min_ESS_per_grad <- model_efficiency_metrics$Min_ess_per_grad_samp_weighted ; min_ESS_per_grad
  ...
  est_time_5000_ESS <- model_fit$time_to_target_ESS(target_ESS = 5000) ; est_time_500_ESS
  est_time_5000_ESS
...
#                                                     save_nuisance_trace = FALSE)
  ...
  lp_grad_outs <-   parallel::mcparallel ( BayesMVP::Rcpp_wrapper_fn_lp_grad( Model_type = Model_type,
                                                                     force_autodiff = F,
                                                                     force_PartialLog = F,
                                                                     multi_attempts = F,
