...
plot_param_group_batched <- function(draws_array, 
              ...
              # Plot in batches
              n_batches <- ceiling(length(param_names) / batch_size)
              
              plots <- list()
              for (i in 1:n_batches) {
                
                        start_idx <- (i-1) * batch_size + 1
                        end_idx <- min(i * batch_size, length(param_names))
                        batch_params <- param_names[start_idx:end_idx]
...
plot_multiple_params_batched <- function(draws_array, 
              ...
              # Get parameter names from array - ADD THIS LINE
