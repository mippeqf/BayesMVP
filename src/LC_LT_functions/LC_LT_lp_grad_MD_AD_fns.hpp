  ...
  
   {
    ...
    for (int nc = 0; nc < n_chunks; nc++) {
      ...
      if ((chunk_counter == n_full_chunks) && (n_chunks > 1) && (last_chunk_size > 0)) { // Last chunk (remainder - don't use AVX / SIMD for this)
                          ...
                          vect_type_tanh = "Loop";
                          vect_type_Phi =  "Loop";
                          vect_type_log_Phi = "Loop";
                          vect_type_inv_Phi = "Loop";
                          vect_type_inv_Phi_approx_from_logit_prob = "Loop";
      ...
      // u_array.array()  = (  u_vec.segment(chunk_size_orig * n_tests * nc , chunk_size * n_tests).reshaped(chunk_size, n_tests).array()   ).array() ;
      ...
      for (int c = 0; c < n_class; c++) {
        ...
        inc_array.array()  = 0.0; // needs to be reset to 0
        ...
        for (int t = 0; t < n_tests; t++) {
                ...
                // Bound_U_Phi_Bound_Z[c].col(t) = fn_EIGEN_double( Bound_Z[c].col(t),   Phi_type, vect_type_Phi,  skip_checks_Phi);
                // Phi_Z[c].col(t).array() = y_chunk.col(t).array() * Bound_U_Phi_Bound_Z[c].col(t).array() +   (y_chunk.col(t).array()   -  Bound_U_Phi_Bound_Z[c].col(t).array()).array() *   ((y_chunk.col(t).array()  + (y_chunk.col(t).array()  - 1.0)) * u_array.col(t).array()).array();
                // Z_std_norm[c].col(t) = fn_EIGEN_double( Phi_Z[c].col(t),   inv_Phi_type, vect_type_inv_Phi,  skip_checks_inv_Phi);
                ...
                // y1_log_prob[c].col(t) = fn_EIGEN_double( prob[c].col(t),  "log", vect_type_log,  skip_checks_log);
      ...
      out_mat.segment(n_params + 1, N).segment(chunk_size_orig * chunk_counter, chunk_size).array()   = fn_log_sum_exp_2d_double(lp_array,  vect_type_lse, skip_checks_lse).array() ;  //  fast_log_sum_exp_2d_AVX512(lp_array);
      prob_n  =  fn_EIGEN_double(out_mat.segment(n_params + 1, N).segment(chunk_size_orig * chunk_counter, chunk_size).matrix(), "exp",  vect_type_exp,  skip_checks_exp);
