...
void         fn_lp_grad_LT_LC_PartialLog_MD_and_AD_InPlace_process(        Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat ,
...

        {
          ...
          for (int c = 0; c < n_class; ++c) {
            for (int t = 0; t < n_tests; ++t) {
              ...
              LT_a_double[c](0, t) = LT_a(c, t).val();
  ...
  ///////////////////////////////////////////////
   ...
   { // start of big local block
    ...
    for (int nc = 0; nc < n_chunks; nc++) {
        ...
        for (int c = 0; c < n_class; c++) {
            ...
            for (int t = 0; t < n_tests; t++) {   // start of t loop
              ...
              if (n_covariates_max > 1) {  //// at present latent_trait only works w/o covariates 
                ...
                Bound_Z[c].col(t).array() =     L_Omega_recip_double[c](t, t) * (  - ( LT_a_double[c](0, t) +      inc_array.array()   )  ) ;
                sign_Bound_Z[c].col(t) =   Bound_Z[c].col(t).array().sign();
                log_abs_Bound_Z[c].col(t) =    (fn_EIGEN_double(Bound_Z[c].col(t).array().abs(), "log", vect_type_log));
              ...
              //// create masks
              overflow_mask =   ( (Bound_Z[c].col(t).array() > overflow_threshold)  && (y_chunk.col(t).cast<double>().array() == 1.0) ).cast<int>().matrix() ;
              underflow_mask =  ( (Bound_Z[c].col(t).array() < underflow_threshold) && (y_chunk.col(t).cast<double>().array() == 0.0) ).cast<int>().matrix() ;
            ...
            fn_MVP_compute_lp_GHK_cols(t, 
                                       Bound_U_Phi_Bound_Z[c],  Phi_Z[c], Z_std_norm[c], prob[c], y1_log_prob[c], // computing these
                                       Bound_Z[c],  y_chunk, u_array, Model_args_as_cpp_struct);   
      ...
      const Eigen::Matrix<double, -1, -1> &log_abs_y_sign_chunk =   (fn_EIGEN_double(y_sign_chunk.array().abs(), "log", vect_type_log)); 
      const Eigen::Matrix<double, -1, -1> &log_abs_y_m_y_sign_x_u =  (fn_EIGEN_double(y_m_y_sign_x_u.array().abs(), "log", vect_type_log));  
      const Eigen::Matrix<double, -1, -1> &sign_y_m_y_sign_x_u = y_m_y_sign_x_u.array().sign();
      ...
      for (int c = 0; c < n_class; c++) {
        ...
        for (int i = 0; i <  Omega_grad_array_for_each_n.size();  i++) {
          ...
          const Eigen::Matrix<double, -1, -1> &y1_log_prob_recip = - y1_log_prob[c].array();
          const Eigen::Matrix<double, -1, -1> &sign_Z_std_norm =  Z_std_norm[c].array().sign();
          const Eigen::Matrix<double, -1, -1> &prob_recip = 1.0 / prob[c].array();
        ...
        const Eigen::Matrix<double, -1, -1> &y_m_ysign_x_u_array_times_phi_Z_times_phi_Bound_Z_times_L_Omega_diag_recip = fn_EIGEN_double(log_abs_y_m_ysign_x_u_array_times_phi_Z_times_phi_Bound_Z_times_L_Omega_diag_recip, "exp", vect_type_exp).array() * 
          ...
          if ( (grad_option == "main_only") || (grad_option == "all") ) {
                 ...
                 if (n_covariates_max > 1) {  ///  not implemented yet for latent_trait
                    ...
                    for (int i = 0; i <  beta_grad_array_for_each_n.size();  i++) {
                      sign_beta_grad_array_for_each_n[i].array() =  beta_grad_array_for_each_n[i].array().sign();
                      log_abs_beta_grad_array_for_each_n[i].array() = fn_EIGEN_double(beta_grad_array_for_each_n[i].array().abs(), "log", vect_type_log);
                      for (int t = 0; t < n_tests; t++) {
          ...
          if ( (grad_option == "main_only") || (grad_option == "all") ) {
             ...
             {  /// entire b-grad computed on log-scale (partial-log-scale not yet implemented)
               ...
               Eigen::Matrix<double, -1, -1> log_abs_grad_bound_z(chunk_size, n_tests);
               Eigen::Matrix<double, -1, -1> sign_grad_bound_z(chunk_size, n_tests);
               Eigen::Matrix<double, -1, -1> log_abs_grad_Phi_bound_z(chunk_size, n_tests);
               Eigen::Matrix<double, -1, -1> sign_grad_Phi_bound_z(chunk_size, n_tests);
