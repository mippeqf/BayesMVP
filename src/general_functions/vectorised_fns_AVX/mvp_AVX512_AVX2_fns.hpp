#ifndef STAN_MATH_PRIM_META_MVP_AVX512_AVX2_FNS_HPP
#define STAN_MATH_PRIM_META_MVP_AVX512_AVX2_FNS_HPP
...
// bookmark (this doesnt seem to be available on g++ at all at time of writing even with all AVX512 flags!)
...
inline __m256d fast_log1p_1_wo_checks_AVX2(const __m256d x) {
    ...
    return _mm256_blendv_pd(
      _mm256_mul_pd(_mm256_fmadd_pd(minus_one_half, x, one), x),
      fast_log_1_wo_checks_AVX2(_mm256_add_pd(x, one)),
      mask
    );
...
inline __m256d fast_log1p_exp_1_wo_checks_AVX2(const __m256d x) {
      ...
      const __m256d neg_x = _mm256_sub_pd(zero, x);
  
      const __m256d mask = _mm256_cmp_pd(x, zero, _CMP_GT_OQ);     // Create mask using _mm256_cmp_pd
...
inline __m256d fast_log_1_AVX2(const __m256d a) {
  ...
  return _mm256_blendv_pd(
    _mm256_blendv_pd(
      ...
      neg_inf,
      _mm256_cmp_pd(zero, a, _CMP_EQ_OQ)
    ),
...
inline __m256d fast_log1p_1_AVX2(const __m256d x) {
  ...
  return _mm256_blendv_pd(
    _mm256_blendv_pd(
      _mm256_mul_pd(_mm256_fmadd_pd(neg_half, x, one), x),
      fast_log_1_AVX2(_mm256_add_pd(x, one)),
...
// inline      __m256d fast_log_1_AVX2(const __m256d a ) {
//   
//             const __mmask8 is_a_finite_and_gr0_mask =  is_finite_mask(a) &  _mm256_cmp_pd_mask(a, zero, _CMP_GT_OQ);
//             
//             return  _mm256_mask_blend_pd(is_a_finite_and_gr0_mask,
//                                          _mm256_mask_blend_pd(_mm256_cmp_pd_mask(zero, a, _CMP_EQ_OQ),     // cond is "if a = 0 "
...
//   
//   const __m256d neg_x =  _mm256_sub_pd(zero, x);
//   const __mmask8 is_x_gr_0_mask =      _mm256_cmp_pd_mask(x,  zero, _CMP_GT_OQ);
//   
...
inline     __m256d  fast_inv_logit_for_x_pos_AVX2(const __m256d x )  {
          ...
          const __m256d  exp_m_x =  fast_exp_1_AVX2(_mm256_sub_pd(zero, x)) ;
...
inline      __m256d  fast_inv_logit_for_x_pos_wo_checks_AVX2(const __m256d x )  {
  ...
  const __m256d  exp_m_x =  fast_exp_1_wo_checks_AVX2(_mm256_sub_pd(zero, x)) ;
  return    _mm256_div_pd(one, _mm256_add_pd(one, exp_m_x))  ;
...
inline     __m256d  fast_log_inv_logit_for_x_pos_AVX2(const __m256d x )  {
    ...
    const    __m256d   m_x = _mm256_sub_pd(zero, x);
    return   _mm256_sub_pd(zero, fast_log1p_exp_1_AVX2(fast_exp_1_AVX2(m_x)));
...
inline     __m256d  fast_log_inv_logit_for_x_pos_wo_checks_AVX2(const __m256d x )  {
   ...
   return   _mm256_sub_pd(zero, fast_log1p_exp_1_wo_checks_AVX2((_mm256_sub_pd(zero, x)) ) );
...
inline      __m256d  fast_inv_Phi_approx_wo_checks_AVX2(const __m256d x )  {
  ...
  return _mm256_mul_pd(  inv_Phi_approx_c2,   _mm256_div_pd(_mm256_sub_pd(_mm256_mul_pd(exp_x_i, exp_x_i), one), exp_x_i) ) ;    //   now do sinh parth part
...
inline     __m256d  fast_inv_Phi_approx_AVX2(const __m256d x )  {
  ...
  const __m256d m_logit_p = fast_log_1_AVX2( _mm256_sub_pd(_mm256_div_pd(one,  x), one ) ); // logit first
  ...
  return _mm256_mul_pd(  _mm256_set1_pd(2.74699999999999988631),   _mm256_div_pd( _mm256_sub_pd(_mm256_mul_pd(exp_x_i, exp_x_i), one), exp_x_i) ) ;    //   now do sinh parth part
...
inline      __m256d    fast_tanh_wo_checks_AVX2(const   __m256d x  )   {
  ...
  return        _mm256_sub_pd( _mm256_div_pd(two,   _mm256_add_pd(one, fast_exp_1_wo_checks_AVX2(  _mm256_mul_pd(x, m_two)   ) ) ), one ) ;
...
inline     __m256d fast_erf_wo_checks_part_1_upper_AVX2(  const __m256d a,
    ...
    r = _mm256_fmadd_pd (erf_part_1_c0, t, erf_part_1_c1); // -0x1.9f363ba3b515dp-58, 0x1.17f6b1d68f44bp-51
    u = _mm256_fmadd_pd (erf_part_1_c2, t, erf_part_1_c3); // -0x1.66b85b7fbd01ap-46, 0x1.22907eebc22e0p-41
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c4, t, erf_part_1_c5); // -0x1.4e0591fd97592p-37, 0x1.21e5e2d8544d1p-33
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c6, t, erf_part_1_c7); // -0x1.8a8f81b7e0e84p-30, 0x1.af85793b93d2fp-27
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c8, t, erf_part_1_c9); // -0x1.8157db0edbfa8p-24, 0x1.1ba3c453738fdp-21
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c10, t, erf_part_1_c11); // -0x1.595999b7e922dp-19, 0x1.5a59c27b3b856p-17
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c12, t, erf_part_1_c13); // -0x1.197b61ee37123p-15, 0x1.631f0597f62b8p-14
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c14, t, erf_part_1_c15); // -0x1.319310dfb8583p-13, 0x1.2d798353da894p-14
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c16, t, erf_part_1_c17); //  0x1.037445e25d3e5p-11,-0x1.a939f51db8c06p-10
    r = _mm256_fmadd_pd (r, s, u);
    u = _mm256_fmadd_pd (erf_part_1_c18, t, erf_part_1_c19); //  0x1.5878d80188695p-13, 0x1.39bc5e0e9e09ap-6
    r = _mm256_fmadd_pd (r, s, u);
    r = _mm256_fmadd_pd (r, t, erf_part_1_c20); // -0x1.a4fbc8f8ff7dap-4
    r = _mm256_fmadd_pd (r, t, erf_part_1_c21); // -0x1.45f2da3ae06f8p-1
    r = _mm256_fmadd_pd (r, t, erf_part_1_c22); // -0x1.06ebb92d9ffa8p-3
    r = _mm256_fmadd_pd (r, t, -t);
    r = _mm256_sub_pd(one, fast_exp_1_AVX2(r)); // r = 1.0 - fast_exp_1_wo_checks_AVX2(r);
...
inline     __m256d fast_erf_wo_checks_part_2_lower_AVX2(const __m256d a,
    ...
    r = _mm256_fmadd_pd (r, s, erf_part_2_c1); //  0x1.d71b0f1b10071p-27
    r = _mm256_fmadd_pd (r, s, erf_part_2_c2); // -0x1.5c0726f04dbc7p-23
    r = _mm256_fmadd_pd (r, s, erf_part_2_c3); //  0x1.b97fd3d9927cap-20
    r = _mm256_fmadd_pd (r, s, erf_part_2_c4); // -0x1.f4ca4d6f3e232p-17
    r = _mm256_fmadd_pd (r, s, erf_part_2_c5); //  0x1.f9a2baa8fedc2p-14
    r = _mm256_fmadd_pd (r, s, erf_part_2_c6); // -0x1.c02db03dd71bbp-11
    r = _mm256_fmadd_pd (r, s, erf_part_2_c7); //  0x1.565bccf92b31ep-8
    r = _mm256_fmadd_pd (r, s, erf_part_2_c8); // -0x1.b82ce311fa94bp-6
    r = _mm256_fmadd_pd (r, s, erf_part_2_c9); //  0x1.ce2f21a040d14p-4
    r = _mm256_fmadd_pd (r, s, erf_part_2_c10); // -0x1.812746b0379bcp-2
    r = _mm256_fmadd_pd (r, s, erf_part_2_c11); //  0x1.06eba8214db68p-3

    return _mm256_fmadd_pd(r, a, a);
...
inline __m256d fast_erf_wo_checks_AVX2(const __m256d a) {

  const __m256d t = _mm256_abs_pd(a);
  const __m256d s = _mm256_mul_pd(a, a);

  return _mm256_blendv_pd(
    fast_erf_wo_checks_part_2_lower_AVX2(a, t, s),
    fast_erf_wo_checks_part_1_upper_AVX2(a, t, s),
    _mm256_cmp_pd(t, one, _CMP_GT_OQ)
  );
...
// AVX2 version with consistent mask logic (i.e., consistent with AVX-512 logic)
...
inline     __m256d fast_inv_erf_wo_checks_part_1_upper_AVX2(const __m256d a, 
    ...
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_1_c2); //  0x1.47e256p-20
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_1_c3); //  0x1.dca7dep-16
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_1_c4); //  0x1.9cab92p-12
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_1_c5); //  0x1.8cc0dep-9
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_1_c6); //  0x1.3ca920p-8
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_1_c7); // -0x1.0eff66p-2
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_1_c8); //  0x1.ae16a4p-1
    
    return  _mm256_mul_pd(a, p); // a * p;
...
inline     __m256d fast_inv_erf_wo_checks_part_2_lower_AVX2(  const __m256d a, 
    ...
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c2); //  0x1.47e256p-20
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c3); //  0x1.dca7dep-16
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c4); //  0x1.9cab92p-12
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c5); //  0x1.8cc0dep-9
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c6); //  0x1.3ca920p-8
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c7); // -0x1.0eff66p-2
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c8); //  0x1.ae16a4p-1
    p = _mm256_fmadd_pd (p, t,  inv_erf_part_2_c9); //  0x1.ae16a4p-1
...
//         t = fast_log_1_AVX2(t);
//         __mmask8 is_abs_t_gr_6p25 =  _mm256_cmp_pd_mask(_mm256_abs_pd(t),  inv_erf_c0, _CMP_GT_OQ); // if x in (0, 1)
//         
//         return   _mm256_mask_blend_pd(is_abs_t_gr_6p25,
