...
Eigen::Matrix<double, -1, 1>  fn_update_tau_w_diag_M_ADAM(   const Eigen::Ref<const Eigen::Matrix<double, -1, 1>>  eigen_vector, 
                                             ...
                                             double &tau,  //// NOT const as updating !!
                                             const double &LR,
                                             const double &ii, 
                                             const double &n_burnin,
                                             const Eigen::Ref<const Eigen::Matrix<double, -1, 1>>  sqrt_M_vec, 
                                             double &tau_m_adam,  //// NOT const as updating !!
                                             double &tau_v_adam, //// NOT const as updating !!
                                             const double &tau_ii
 ) {
   ...
   if (ii > eta_w && eigen_max > 0) {
       ...
       //////////  ------------- END OF PARALLEL PART (take averages of tau_noisy_grad across the K chains) --------------------------------
         ...
         tau_m_adam = beta1_adam * tau_m_adam + (1.0 - beta1_adam) * tau_noisy_grad;
         tau_v_adam = beta2_adam * tau_v_adam + (1.0 - beta2_adam) * std::pow(tau_noisy_grad, 2);
         double tau_m_hat = tau_m_adam / (1.0 - std::pow(beta1_adam, ii));
         double tau_v_hat = tau_v_adam / (1.0 - std::pow(beta2_adam, ii));
         
         double current_alpha = LR * (1.0 - ((1.0 - LR) * ii) / n_burnin);
         double log_tau = std::log(stan::math::fabs(tau)) + current_alpha * tau_m_hat / (std::sqrt(tau_v_hat) + eps_adam);
         tau = std::exp(log_tau);
...
Eigen::Matrix<double, -1, 1> fn_update_tau_w_dense_M_ADAM(  const Eigen::Ref<const Eigen::Matrix<double, -1, 1>>  eigen_vector, 
  ...
  const double rho = 1.0;
    ...
    Eigen::Matrix<double, -1, 1> out_vec(3);
    out_vec(0) = tau;
    out_vec(1) = tau_m_adam;
    out_vec(2) = tau_v_adam;
