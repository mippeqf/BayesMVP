...
inline auto fn_MVP_compute_phi_Bound_Z_cols(       const int t,
   ...
   const std::string &vect_type_exp = Model_args_as_cpp_struct.Model_args_strings(3);
     ...
     if ( (Phi_type == "Phi_approx") || (Phi_type == "Phi_approx_2") ) { // vect_type
       ...
       phi_Bound_Z.col(t).array()  =         sqrt_2_pi_recip * fn_EIGEN_double( ( - 0.5 * Bound_Z.col(t).array().square() ).matrix(),  "exp", vect_type_exp);
     }
...
// fn that computes phi_Z_recip needed for the gradient of the GHK parameterisation of the MVP / LC_MVP (and latent_trait) models
inline auto fn_MVP_compute_phi_Z_recip_cols(       const int t,
