...
void                             fn_lp_grad_MVP_LC_Pinkney_NoLog_MD_and_AD_INNER_Inplace_process(    Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat,
                                                                                                     ...
                                                                                                     const std::vector<Eigen::Matrix<stan::math::var, -1, -1>> &Omega_unconstrained_var,
                                                                                                     std::vector<Eigen::Matrix<stan::math::var, -1, -1>> &L_Omega_var,
                                                                                                     const std::vector<Eigen::Matrix<double, -1, -1>> &L_Omega_double,
                                                                                                     const std::vector<Eigen::Matrix<double, -1, -1>> &L_Omega_recip_double,
                                                                                                     const Eigen::Ref<const Eigen::Matrix<double, -1, -1>> prev,
                                                                                                     const double log_det_J_main, 
                                                                                                     const double prior_densities, 
...
//  const bool corr_force_positive = Model_args_as_cpp_struct.Model_args_bools(2);
  ...
  /////////////  some prev stuff (gradient of log_det_J)
  const double u_prev_diseased = stan::math::atanh(2.0*prev(0, 1) - 1.0);
 ...
 //   prev_var_vec[1] =  0.5 * ( tanh_u_prev[1] + 1.0);
  //  prev_var_vec[0] =  0.5 * ( tanh_u_prev[0] + 1.0);
   ...
   // prev(0,0) =  prev_var_vec[0];
    ...
    // log_jac_p =    log( deriv_p_wrt_pu_double );

  }



  ///////////  gradient of log_det_J  for L_Omega using autodiff - for L_Omega -> Omega_raw transformation / gradient of log_det_J w.r.t Omega_raw
...

  {
    ...
    Eigen::Matrix<double, -1, -1>   phi_Z_recip =   Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests);
    Eigen::Matrix<double, -1, -1>   phi_Bound_Z =   Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests);
    ///////////////////////////////////////////////
    ...
    ///////////////////////////////////////////////
    Eigen::Matrix<double, -1, -1>   u_grad_array_CM_chunk_block =  Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests);
    ///////////////////////////////////////////////
    ...
    { // start of big local block
      ...
      for (int nc = 0; nc < n_total_chunks; nc++) {
        ...
        u_vec_chunk =   fn_MVP_compute_nuisance(    u_vec_chunk, u_unc_vec_chunk, Model_args_as_cpp_struct);
            ...
            prod_container_or_inc_array.setZero(); /// reset to 0
                ...
                for (int t = 0; t < n_tests; t++) {
                        ...
                        fn_MVP_compute_lp_GHK_cols(t,
                                                   ...
                                                   y_chunk,
                                                   u_array,
                                                   Model_args_as_cpp_struct);  //// compute_grad_quantities
        ...
        for (int c = 0; c < n_class; c++) {
          ...
          for (int t = 0; t < n_tests; t++) {
                ...
                phi_Z_recip.col(t) = fn_MVP_compute_phi_Z_recip_cols(   t,
                                                                        phi_Z_recip, // computing this
                                                                        Phi_Z[c],
                                                                        Z_std_norm[c],
                                                                        Model_args_as_cpp_struct);

                phi_Bound_Z.col(t) = fn_MVP_compute_phi_Bound_Z_cols(   t,
...
void                             fn_lp_grad_MVP_LC_Pinkney_NoLog_MD_and_AD_Inplace_process(    Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat ,
 ...
 //  
  ...
  ////////  ------- call likelihood ("inner") function  --------------------------------------------------------------------------------------------------------------------------
  ...
  fn_lp_grad_MVP_LC_Pinkney_NoLog_MD_and_AD_INNER_Inplace_process(out_mat,
                                                                  y_ref,
                                                                  theta_us_vec_ref,
                                                                  beta_double_array,
                                                                  Omega_unconstrained_var,
                                                                  L_Omega_var,
                                                                  L_Omega_double,
